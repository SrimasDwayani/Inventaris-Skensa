<?php 
	...
	$query= "SELECT * FROM tbsm";

	$hasil = mysqli_query($conn, $query);
	if (isset($_POST['cari'])) {
		$cari= $_POST['keywoard'];
		$hasil  = mysqli_query($conn, "SELECT * FROM tbsm WHERE nama_barang like '%$cari%' ");
	}
 				...
 				while ($data = mysqli_fetch_array($hasil)) {
 					?> 
 						<tr>
 							...
 							<td>
 							<a class="a" href="edit_tbsm.php?id= <?php echo $data['id_inventaris']; ?>">Edit</a>  |
 							<a class="a" href="hapus_tbsm.php?id= <?php echo $data['id_inventaris'] ?>" onclick = "return confirm(Apakah Anda yakin ingin menghapus <?php echo $data['nama_barang']; ?>)">Hapus</a>
 						</td>
 						</tr>
 					<?php $nomor++; 
 				}
